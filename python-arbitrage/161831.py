...
XSHE_161831_P_NET_VALUE = 0.8769

SLEEP_INTERVAL = 5


def calc_estimated_net_value(lof_data, hscei_data):
    nv = XSHE_161831_P_NET_VALUE * hscei_data.close_price / hscei_data.prev_close_price
    nv *= (1 + HKDCYNC / 100.0)
    return nv
...
def run_calc():
    ...
    lof_md = marketdata[XSHE_161831]
    hscei_md = marketdata[HSCEI]

    net_value_est = calc_estimated_net_value(lof_md, hscei_md)
    return_est = calc_estimated_return(XSHE_161831_P_NET_VALUE, net_value_est, lof_md)
    print(data_format_str % (lof_md.ticker, lof_md.prev_close_price, lof_md.close_price, lof_md.get_return() * 100.0,
                             XSHE_161831_P_NET_VALUE, net_value_est, (net_value_est / XSHE_161831_P_NET_VALUE - 1) * 100.0,
...
if __name__ == '__main__':
    # get_marketdata([XSHE_161831, HSCEI])
    main()
